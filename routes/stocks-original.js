...
// FIXED: Get all stocks with full data (for intraday analysis)
router.get('/list', async (req, res) => {
  try {
    console.log('📋 Fetching all stocks for analysis...');
    ...
    if (snapshot.empty) {
      ...
      return res.json({ 
        success: true, 
        data: [],
        message: 'No stocks found in database. Please import stock data first.'
      });
    }
    ...
    snapshot.forEach(doc => {
      ...
      // Return full stock object with all required fields for analysis
      stocks.push({
        id: doc.id,
        symbol: data.symbol,
        companyName: data.companyName || data.name || '', // Handle both field names
        sector: data.sector || 'Unknown',
        industry: data.industry || 'Unknown',
        basicIndustry: data.basicIndustry || '',
        macroEconomicClassification: data.macroEconomicClassification || '',
        marketCap: data.marketCap || 0,
        freeFloatMarketCap: data.freeFloatMarketCap || 0,
        // Default values for intraday analysis (these will be updated with real-time data)
        price: data.lastPrice || data.price || 0,
        change: data.change || 0,
        changePercent: data.changePercent || data.pChange || 0,
        volume: data.volume || data.finalQuantity || 0,
    ...
    res.json({ 
      ...
      data: stocks,
      count: stocks.length,
      message: `Successfully loaded ${stocks.length} stocks for analysis`
    });
    ...
    res.status(500).json({ 
      success: false, 
      error: error.message,
      message: 'Failed to load stocks. Please check database connection.'
    });
  }
});

// Additional endpoint for symbols only (if needed elsewhere)
router.get('/symbols', async (req, res) => {
  try {
    ...
    const symbols = snapshot.docs.map(doc => doc.data().symbol).filter(symbol => symbol);

    res.json({ success: true, data: symbols.sort() });
  } catch (error) {
